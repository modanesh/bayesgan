...
path = '/home/saeed/workspace/Project_PGM/resultsn/bgan_svhn_1553115373/'

files = os.listdir(path)


max_iter = 20000
for i in range(100, max_iter, 100):
    raw_images = [x for x in files if x.startswith('RAW') and x.endswith('.png') and len(x) > 12 and '_'+str(i)+'_' in x]
    gen_images = [x for x in files if x.startswith('B_DCGAN') and x.endswith('.png') and len(x) > 24 and '_'+str(i)+'_' in x]
    couple = zip(raw_images, gen_images)
    raw_img = np.zeros((25,3,32,32))
    gen_img = np.zeros((25,3,32,32))
    for ind, item in enumerate(couple):
        raw, gen = item
        raw_img[ind,:,:,:] = np.transpose(plt.imread(os.path.join(path, raw), 'RGB'), (2, 0, 1)) * 255
        gen_img[ind,:,:,:] = np.transpose(plt.imread(os.path.join(path, gen), 'RGB'), (2, 0, 1)) * 255
        print('fid score in iteration ', i, ': ', get_fid(raw_img, gen_img))
