...
def bgan_synth(synth_dataset, z_dim, batch_size=64, numz=5, num_iter=1000, 
    ...
    for train_iter in range(num_train_iter):
        ...
        for gi in range(bgan.num_gen):
            ...
            _, g_loss, prior = session.run([bgan.g_optims[gi], bgan.generation["g_losses"][gi], bgan.generation["g_prior"][gi]], 
        ...
        if (train_iter + 1) % 200 == 0:
            ...
            for i in range(int(synth_dataset.N_test/batch_size)):
                d_logit = session.run([bgan.D], feed_dict={bgan.inputs: synth_dataset.X_test[i*batch_size:(i+1)*batch_size]})
                d_logit = d_logit[0]
...
if __name__ == "__main__":
    ...
    results_path = os.path.join(args.out_dir, "experiment_%i_test" % (int(time.time())))
    ...
    synth_d = SynthDataset_generated(path = os.path.join(args.out_dir, "dataset_prior_zuniform/run_regular_bayes.npz" ))

    results = bgan_synth(synth_d, synth_d.z_dim, num_iter=args.train_iter, 
